...
func (s *Server) runAppInstall(appID string, def *ServerAppDef, port int) {
        ...
        // Validate the executable against an allowlist of known-safe commands to
        // prevent arbitrary binary execution if InstallCommand is ever tampered with.
        ...
        execBase := filepath.Base(argv[0])
        if !allowedExecutables[execBase] {
                setStatus("error", "", "install command uses disallowed executable: "+execBase)
                return
        }

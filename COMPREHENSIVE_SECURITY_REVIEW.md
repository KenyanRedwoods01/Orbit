...
|Session fixation|✅ Mitigated|JWT regenerated on login; `orbit\_session` cookie set with `HttpOnly`, `SameSite=Strict`|
...
|Null pw\_hash edge case|✅ FIXED|Guard added in `auth.go:143` — rejects empty password hash before bcrypt|
...
|JWT no expiry|✅ Present|24h TTL in `auth.go:148`; server-side `sessions.expires\_at` column|
...
|Command injection|✅ FIXED|`bash -c` eliminated in github\_actions.go, wazuh.go, apps.go; regex validation on commands|
...
|Running commands as root unnecessarily|✅ Mitigated|Docker runs with `cap\_drop: ALL`; binary owned by `orbit:orbit` in Dockerfile|
...
|No host verification|✅ FIXED|`sshHostKeyCallback` in `multiserver.go` — now uses known\_hosts with fallback logging fingerprint|
...
|Containers privileged|✅ Mitigated|`cap\_drop: ALL`; `--security-opt no-new-privileges`|
...
|SYS\_PTRACE + NET\_ADMIN capabilities|✅ FIXED|SYS\_PTRACE removed (gopsutil reads /proc directly); NET\_ADMIN retained for firewall|
...
|Check|Status|Evidence / Fix|
|-|-|-|
|No audit logs|✅ Present|`audit.go` tracks actions; `audit\_log` table with actor/target/action/ip|
...
|No alerting|✅ Present|`alert\_rules.go` with metric-based alerting; notifications module|
|No brute-force detection|✅ FIXED|Rate limiters + audit log for failed auth|
|No command execution logs|✅ Present|Audit log with action field; github\_actions runs logged|
...
|GitHub Actions takeover|⚠️ Needs audit|`pull\_request\_target` usage, action pinning, token scope|
...
|SYS\_PTRACE claimed necessary|✅ FIXED|Removed — gopsutil reads /proc, no ptrace needed (Batch 14)|
...
|Container escape risk|⚠️ WARNING|SYS\_PTRACE + NET\_ADMIN capabilities; documented warning added|
...
|Can execute shell commands from UI without restrictions|✅ FIXED|Command validation in cron, github\_actions, wazuh; arg-based exec everywhere|
...
|4|DB fixes|Migration error logging (non-duplicate); empty pw\_hash guard in auth.go line 143|
...
|9|Null pw\_hash|Edge case guard in profile.go (2x), users.go, auth.go (1x)|
